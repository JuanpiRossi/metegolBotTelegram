...
def _calculate_elo(elo_player_a, elo_player_b, player_a_goals, player_b_goals):
    ...
    player_b_transformed = float(10**(float(elo_player_a)/400))

    player_a_expect_score = float(player_a_transformed) / float(player_a_transformed+player_b_transformed)
    player_b_expect_score = float(player_b_transformed) / float(player_b_transformed+player_b_transformed)
    ...
    new_player_a_elo = float(elo_player_a + goals_multiplier*player_constant*(float(player_a_goals>player_b_goals)-player_a_expect_score))
    new_player_b_elo = float(elo_player_b + goals_multiplier*player_constant*(float(player_b_goals>player_a_goals)-player_b_expect_score))
